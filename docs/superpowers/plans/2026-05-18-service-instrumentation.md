...
- [x] **Scrape opt-out for the three APIs** (done 2026-08-11, after confirming OTLP records carrying `scope_name=@opentelemetry/winston-transport` on both clusters). `logs.grafana.com/pods.enabled: "false"` in `podAnnotations` — *not* `annotations`, which lands on the workload where Alloy never looks. `qr-manager-ui` stays scraped: nginx, no SDK. Crash/boot-failure coverage for the three APIs is given up; that is the accepted cost.
