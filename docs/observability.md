...
Five panels read alarming and are reporting the truth about a deliberate (or at least known) configuration. Documented so nobody debugs them twice:

- **"Disk space allocated" sits near or above 100% on busy volumes, and that is not a full disk.** `longhorn_volume_actual_size_bytes` counts blocks allocated, not bytes in use. Longhorn thin-provisions and never releases a block once written unless a filesystem-trim job runs, and snapshots count toward it, so any write-churning volume ratchets up and stays there — it can exceed 100% because snapshot and metadata overhead sits outside the nominal capacity. Measured 2026-08-14: EMQX 100.2% allocated against **7.7% filesystem used**, Prometheus 94.7% against **27.0%**. The panel that answers "is this volume filling up" is **Filesystem used**, from `kubelet_volume_stats_*`. Note the two cannot share a legend — the kubelet labels are `namespace` / `persistentvolumeclaim`, Longhorn's are `pvc_namespace` / `pvc`.
- **Degraded volumes is non-zero on every cluster, and Healthy volumes reads 0.** The `longhorn` StorageClass requests `numberOfReplicas: "3"` while each cluster is a single node, so exactly one replica can ever be scheduled and every volume sits at `robustness: degraded` forever. `defaultSettings.defaultReplicaCount: 1` does **not** fix this — the StorageClass is rendered from the chart's `persistence.*` keys, and a parameter set there wins over the global default setting. [iac/clusters/helm-values/longhorn.yaml](../iac/clusters/helm-values/longhorn.yaml) now carries `persistence.defaultClassReplicaCount: 1`; it takes effect on `terraform apply` per cluster, and volumes that already exist additionally need `spec.numberOfReplicas` patched, since the StorageClass only governs new ones.
