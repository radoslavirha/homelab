...
| qr-manager-ui | QR code admin SPA (React + nginx); served at `apps.server1.homelab.irha.cz/qr-manager`; runtime `config.json` via ConfigMap subPath mount; no secrets | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/qr-manager-ui/base.yaml) · [production](../gitops/helm-values/server1/apps/qr-manager-ui/production.yaml) · [sandbox](../gitops/helm-values/server1/apps/qr-manager-ui/sandbox.yaml) · [shared](../gitops/helm-values/server1/apps/values.yaml) · [appset](../gitops/argocd-manifests/apps/apps/QrManagerUi.yaml) | — |
