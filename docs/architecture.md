...
| app | Shared Helm chart for our custom apps, one app per release; config files rendered by ESO (variables + OpenBao secrets), secretRefs, optional Argo Rollouts | server1 | ArgoCD `apps` | — | [chart](../gitops/helm-charts/app/) | — |
| miot-bridge-api | MIOT device bridge API; HTTP ingress; MQTT + MongoDB; auto-provisioned credentials via PostSync Jobs | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/miot-bridge-api/values.yaml) · [production](../gitops/helm-values/server1/apps/miot-bridge-api/values-production.yaml) · [sandbox](../gitops/helm-values/server1/apps/miot-bridge-api/values-sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/MiotBridgeApi.yaml) | — |
| interactive-map-feeder-api | Interactive map feeder API; HTTP ingress only; no secrets | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/interactive-map-feeder-api/values.yaml) · [production](../gitops/helm-values/server1/apps/interactive-map-feeder-api/values-production.yaml) · [sandbox](../gitops/helm-values/server1/apps/interactive-map-feeder-api/values-sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/InteractiveMapFeederApi.yaml) | — |
| qr-manager-api | QR code redirect + admin CRUD API; HTTP ingress + `qr.irha.cz` shortcut; MongoDB for slug storage; auto-provisioned MongoDB credentials via PostSync Jobs | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/qr-manager-api/values.yaml) · [production](../gitops/helm-values/server1/apps/qr-manager-api/values-production.yaml) · [sandbox](../gitops/helm-values/server1/apps/qr-manager-api/values-sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/QrManagerApi.yaml) | — |
| homelab-dashboard-ui | Homelab landing page (React + nginx) at `dashboard.server3.homelab.irha.cz`; rendered by the `app` chart; OIDC via Authentik | server3 | ArgoCD `server3/dashboards` | — | [server3](../gitops/helm-values/server3/homelab-dashboard-ui/values.yaml) | — |
| qr-manager-ui | QR code admin SPA (React + nginx); served at `apps.server1.homelab.irha.cz/qr-manager`; runtime `config.json` via ConfigMap subPath mount; no secrets | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/qr-manager-ui/values.yaml) · [production](../gitops/helm-values/server1/apps/qr-manager-ui/values-production.yaml) · [sandbox](../gitops/helm-values/server1/apps/qr-manager-ui/values-sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/QrManagerUi.yaml) | — |
...
App routes generated by the `app` chart follow the same rule, with the stage label
