...
| iot-applications | Shared Helm chart for custom IoT apps; supports multi-app deployments, config files rendered by ESO (variables + OpenBao secrets), secretRefs, optional Argo Rollouts | server1 | ArgoCD `apps` | — | [chart](../gitops/helm-charts/iot-applications/) | — |
| miot-bridge-api | MIOT device bridge API; HTTP ingress; MQTT + MongoDB; auto-provisioned credentials via PostSync Jobs | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/miot-bridge-api/base.yaml) · [production](../gitops/helm-values/server1/apps/miot-bridge-api/production.yaml) · [sandbox](../gitops/helm-values/server1/apps/miot-bridge-api/sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/MiotBridgeApi.yaml) | — |
| interactive-map-feeder-api | Interactive map feeder API; HTTP ingress only; no secrets | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/interactive-map-feeder-api/base.yaml) · [production](../gitops/helm-values/server1/apps/interactive-map-feeder-api/production.yaml) · [sandbox](../gitops/helm-values/server1/apps/interactive-map-feeder-api/sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/InteractiveMapFeederApi.yaml) | — |
| qr-manager-api | QR code redirect + admin CRUD API; HTTP ingress + `qr.irha.cz` shortcut; MongoDB for slug storage; auto-provisioned MongoDB credentials via PostSync Jobs | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/qr-manager-api/base.yaml) · [production](../gitops/helm-values/server1/apps/qr-manager-api/production.yaml) · [sandbox](../gitops/helm-values/server1/apps/qr-manager-api/sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/QrManagerApi.yaml) | — |
...
| qr-manager-ui | QR code admin SPA (React + nginx); served at `apps.server1.homelab.irha.cz/qr-manager`; runtime `config.json` via ConfigMap subPath mount; no secrets | server1 | ArgoCD `apps` | — | [base](../gitops/helm-values/server1/apps/qr-manager-ui/base.yaml) · [production](../gitops/helm-values/server1/apps/qr-manager-ui/production.yaml) · [sandbox](../gitops/helm-values/server1/apps/qr-manager-ui/sandbox.yaml) · [vars](../gitops/helm-values/server1/apps/vars/) · [appset](../gitops/argocd-manifests/apps/apps/QrManagerUi.yaml) | — |
