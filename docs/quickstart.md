...
|3|`RootObservability`, `server3/RootObservability`, `server3/RootIdentity`, `RootIoT`, `RootDatabases`, `RootDashboards`|k8s-monitoring · LGTM · Authentik · IoT · MongoDB · UI dashboards|
|4|`RootApps`|Custom apps (miot-bridge, interactive-map-feeder, qr-manager)|

### Step 7 — OpenBao configuration (after Authentik is up)

Terraform `vault-config` stage; today it adds OIDC login to OpenBao via Authentik. **It cannot run
earlier**: the OIDC client secret is generated by Authentik, so `authentik-server3` must be Healthy and
its blueprint must have created the `openbao-server3-production` provider (~15 min after sync).

```bash
# Admin session — userpass or root, never oidc (this step creates it):
...
Then add yourself to `openbao-server3-production-admin` in Authentik and log in on
`https://vault.server3.homelab.irha.cz/ui/` with method **OIDC** — see [identity.md](identity.md#openbao).
...
argocd login argocd.server3.homelab.irha.cz --username admin --password <plaintext-admin-password> --grpc-web
