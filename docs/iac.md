...
Terraform manages five stages for the **server3** cluster (bootstrap, platform, vault, apps, vault-config) and two stages for **server1** and **server2**.
...
iac/
  modules/
    ...
    vault-config/       OpenBao's own configuration via the vault provider (auth methods, policies,
                        identity groups). Today: OIDC login via Authentik
  clusters/
...
# Keep this even with OIDC: Authentik's secrets come from OpenBao, so OpenBao must stay
# administrable without Authentik. OIDC login via Authentik is added later, in step 6.
...
# 6. OpenBao configuration — Terraform `vault-config` stage (today: OIDC login via Authentik)
#    ⚠️  Only AFTER step 5: authentik-server3 must be Healthy and its blueprint must have created
#    the openbao-server3-production provider (~15 min after sync, see docs/identity.md →
#    "Operational notes"). The OIDC client secret is GENERATED by Authentik — this step cannot
#    move earlier, and nothing that must exist before ArgoCD belongs in this stage.
#    Admin session: userpass (3.g) or root, never oidc, which is what this step creates.
...
# Client secret: Authentik → Applications → Providers → openbao-server3-production → Edit
read -rs S && bao kv put secret/server3/openbao oidc-client-secret="$S"; unset S

cd iac/clusters/server3/vault-config && terraform init && terraform plan && terraform apply

# Then add yourself to openbao-server3-production-admin in Authentik and verify the login:
# docs/identity.md → "OpenBao".
```
...
### modules/vault-config

Provider `hashicorp/vault` `5.x` and Terraform `>= 1.11`. The client secret is read with the ephemeral
`vault_kv_secret_v2` and passed as the write-only `oidc_client_secret_wo`, so it **never enters state**.

| Variable | Type | Default | Description |
|----------|------|---------|-------------|
| `oidc_mount_path` | string | `"oidc"` | Auth mount path. The UI callback embeds it: change `oidc_redirect_uris` and the blueprint with it |
| `oidc_discovery_url` | string | — | Authentik per-provider issuer, `https://auth.irha.cz/application/o/<client_id>/` |
| `oidc_client_id` | string | — | Authentik client_id; also the bound audience |
| `oidc_client_secret_path` | string | — | KV v2 path (mount `secret`) holding key `oidc-client-secret`, e.g. `server3/openbao` |
| `oidc_client_secret_version` | number | — | Version of the write-only secret. **Bump after rotating the value in KV**: it is never read back, so plan cannot see the change |
| `oidc_redirect_uris` | list(string) | — | Must match Authentik's registered URIs exactly |
| `oidc_role_name` | string | `"authentik"` | The single OIDC role; also the mount's `default_role` |
| `oidc_token_ttl` | number | `3600` | Token TTL, seconds |
| `oidc_token_max_ttl` | number | `28800` | Token max TTL, seconds |
| `oidc_groups` | map(object) | — | `roles` claim value → `{ policy_name, policy }`. One ACL policy, one external group and one group alias per entry |

Outputs: `oidc_mount_accessor`
