...
**Required before:** `iac/clusters/server3/vault-config` (`terraform apply`), which is the last step of a
server3 build ([iac.md](iac.md) step 6).

Seed it only **after** Authentik is up: the value is generated by Authentik. It is the client secret of
the `openbao-server3-production` provider, presented by OpenBao's OIDC login. Terraform reads it
ephemerally, so it never lands in Terraform state.

```bash
# Authentik → Applications → Providers → openbao-server3-production → Edit → Client Secret
read -rs S && bao kv put secret/server3/openbao oidc-client-secret="$S"; unset S

# Verify (metadata only, not the value)
bao kv metadata get secret/server3/openbao
```

Rotating it also means bumping `oidc_client_secret_version`. See [identity.md](identity.md#openbao).
...
| --- | --- |
| miot-bridge-api · qr-manager-api · qr-manager-ui · interactive-map-feeder-api | `gitops/helm-values/server1/apps/<app>/values.yaml` → `annotations` |
| homelab-dashboard-ui | `gitops/helm-values/server3/homelab-dashboard-ui/values.yaml` → `annotations` |
