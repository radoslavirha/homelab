...
- Image repo + tag: `gitops/helm-values/server1/apps/<app>/base.yaml` → `apps.<key>.image.repository` + `.tag`
- Component (subdomain): `gitops/helm-values/server1/apps/<app>/base.yaml` → `apps.<key>.labels.component`
- HTTP path: `gitops/helm-values/server1/apps/<app>/base.yaml` → `apps.<key>.ingress.pathName`
